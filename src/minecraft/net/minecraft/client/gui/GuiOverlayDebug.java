...
public class GuiOverlayDebug extends Gui
{
    ...
    private long lastPressedButton;
    /** CODE CUSTOM*/
    ...
    protected List<String> call()
    {
        ...
        else
        {
            ...
            List<String> list = Lists.newArrayList(TextFormatting.GREEN + "StudyCorp Client 1.12.2", this.mc.debug,  this.mc.renderGlobal.getDebugInfoEntities(),  this.mc.world.getProviderName(), "", String.format("X: %.3f", this.mc.getRenderViewEntity().posX), String.format("Y: %.5f", this.mc.getRenderViewEntity().getEntityBoundingBox().minY), String.format("Z: %.3f", this.mc.getRenderViewEntity().posZ),  String.format("Direction : %s", enumfacing, s, MathHelper.wrapDegrees(entity.rotationYaw), MathHelper.wrapDegrees(entity.rotationPitch)));
            ...
            if (this.mc.world != null)
            {
                ...
                if (this.mc.world.isBlockLoaded(blockpos) && blockpos.getY() >= 0 && blockpos.getY() < 256)
                {
                    if (!chunk.isEmpty())
                    {
                        ...
                        list.add("Luminosité: " + chunk.getLightSubtracted(blockpos, 0));
                        //CODE CPS
                        TextFormatting CPSColor = TextFormatting.WHITE;

                        if (getCPS() == 0)
                        {
                            CPSColor = TextFormatting.WHITE;
                        }
                        else if (getCPS() <= 3)
                        {
                            CPSColor = TextFormatting.GREEN;
                        }
                        else if (getCPS() <= 7)
                        {
                            CPSColor = TextFormatting.YELLOW;
                        }
                        else if (getCPS() <= 14)
                        {
                            CPSColor = TextFormatting.RED;
                        }

                        String displayCPS = CPSColor + "CPS: " + getCPS();
                        final boolean pressed = Mouse.isButtonDown(0);

                        if (pressed != this.wasPressed)
                        {
                            this.lastPressedButton = System.currentTimeMillis();
                            this.wasPressed = pressed;

                            if (pressed)
                            {
                                this.clicks.add(this.lastPressedButton);
                            }
                        }
            ...
            if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK && this.mc.objectMouseOver.getBlockPos() != null)
            {
                ...
                list.add(String.format("Regards vers: %d %d %d", blockpos1.getX(), blockpos1.getY(), blockpos1.getZ()));
            }
    ...
    protected <T extends Comparable<T>> List<String> getDebugInfoRight()
    {
        ...
        long l = j - k;
        List<String> list = Lists.newArrayList(String.format("Java: %s %dbit", System.getProperty("java.version"), this.mc.isJava64bit() ? 64 : 32), String.format("Memoire: % 2d%% %03d/%03dMB", l * 100L / i, bytesToMb(l), bytesToMb(i)));
